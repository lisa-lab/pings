	...
	*/
	...
	public void initialize() {
		...
		if (!spherical) {
			...
			if (secant) {
				n = Math.log(m1 /
				   MapMath.msfn(sinphi = Math.sin(standardLatitude2), Math.cos(standardLatitude2), es));
			...
			if (secant)
				n = Math.log(cosphi / Math.cos(standardLatitude2)) /
				   Math.log(Math.tan(MapMath.QUARTERPI + .5 * standardLatitude2) /
				   Math.tan(MapMath.QUARTERPI + .5 * standardLatitude1));
			c = cosphi * Math.pow(Math.tan(MapMath.QUARTERPI + .5 * standardLatitude1), n) / n;
			rho0 = (Math.abs(Math.abs(projectionLatitude) - MapMath.HALFPI) < 1e-10) ? 0. :
